...
class EnvKitchenSceneEnvWithObjRandTesolo(BaseEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.physical_embeds = torch.from_numpy(physical_prop_embed).to(device="cuda" if num_envs > 1 else "cpu")
        ...
        self.load_all_objects = all_objects
    ...
    def step(self, action, *args, **kwargs):
        ...
        residual_limit = torch.tensor(
            [0.15, 0.10, 0.15, 1.0, 1.0, 1.0], device=self.device
        )

        # residual_limit = torch.tensor(
        #     [0.15, 0.15, 0.15, 1.0, 1.0, 1.0], device=self.device
        # )
        action[:, :6] = (
            action[:, :6]
            * 2
        ...
        if self.individual_fingers:
            ...
            thumb_action = action[:, [6]] 
            index_action = action[:, [7]]
            middle_action = action[:, [8]]
            ring_action = action[:, [9]]
            pinky_action = action[:, [10]]

            action[:, [6, 7, 8, 9]] = thumb_action
            action[:, [10, 11, 12, 13]] = index_action
            action[:, [14, 15, 16, 17]] = middle_action
            action[:, [18, 19, 20, 21]] = ring_action
            action[:, [22, 23, 24, 25]] = pinky_action
        else:
            action[:, 6:] = action[:, 6][..., None] * 2
            # To ensure finger are spread out
            # action[:, 10] = -2
            # action[:, 18] = 2
            # action[:, 23] = 2
        ...
        # print(total_action[0, :6])

        # total_action[:, 6:] = torch.clamp(total_action[:, 6:], min=self.tesollo_start, max=torch.ones_like(self.tesollo_start)) 
        ...
        # if self.cur_step:
        #    self.print_current_pairwise_contacts(self.all_objects[self.target_obj_name +  "_0"])
        #    breakpoint()


        # self.print_current_pairwise_self_contacts_agent()
        # breakpoint()
    ...
    def _load_agent(self, options: dict):
        ...
        # self.agent.disable_intra_finger_contacts()
    ...
    def _load_scene(self, options: dict):
        if self.scene_builder.build_configs is not None:
            self.scene_builder.build(
                ...
                selected_objs=None if self.load_all_objects else self.selected_objs,
                selected_arts=None if self.load_all_objects else self.selected_arts,
    ...
    def compute_contact_forces(self):
        ...
        finger_links = self.agent.finger_tip_links
    ...
    def _get_obs_extra(self, info):
        ...
        if not self.is_original:
            if self.include_physical_embeds:
                ...
                obs["physical_embed"] = torch.ones_like(self.physical_embeds.clone()) * 0.5
    ...
    def _set_start_joints(self, env_idx):
        ...
        if self.load_trajectories:
            hand_offset = torch.tensor([self.hand_offset_x, 0, 0], 
                                  dtype=self.trajectories["hand"].dtype, 
                                  device=self.trajectories["hand"].device)

            target_hand_pos = self.cur_traj["hand"][env_idx, 0].clone()
            
            hand_offset = rearrange(hand_offset, "dim -> 1 dim")
            
            target_hand_pos[:, :3] += hand_offset

            start_qpos[:, :6] = target_hand_pos 
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        with torch.device(self.device):
            ...
            for name in self.trajectories.keys():
                ...
                #     cutting_board_offset = torch.tensor([-0.075, 0, 0], dtype=self.trajectories[name].dtype, device=self.trajectories[name].device)
                #     self.cur_traj["cutting board"][env_idx] += rearrange(cutting_board_offset, "dim -> 1 1 dim")
                ...
                if name == "hand":
                    ...
                    hand_offset = rearrange(hand_offset, "dim -> 1 1 dim")
                    self.cur_traj[name][env_idx][:, :, :3] += hand_offset
            ...
            if self.load_trajectories:
                for name in self.random_objs:
                    ...
                    obj_pose = self.object_poses[name][self.traj_idx[env_idx]].clone()

                    # Jayson's code ----
                    if name == self.target_obj_name + "_0":
                        obj_pose[:, :3] += offset[env_idx].to(dtype=obj_pose.dtype, device=obj_pose.device)

                    # if name == "cutting_board_0":
                    #     cutting_board_offset = torch.tensor([-0.075, 0, 0], dtype=obj_pose.dtype, device=obj_pose.device)
                    #     obj_pose[:, :3] += rearrange(cutting_board_offset, "dim -> 1 dim")
    ...
    def is_success(self):
        raise NotImplementedError(
            "is_success() method should be implemented in the subclass."
        )

    def get_spawn_failures(self):
        name = self.target_obj_name + "_0"
        orientation = self.all_objects[name].pose.q.cpu().numpy() 
        orientation = R.from_quat(orientation, scalar_first=True)

        initial_orientation = R.from_quat(self.object_poses[name][self.traj_idx].cpu().numpy()[:, 3:], scalar_first=True)

        rel_pos = orientation * initial_orientation.inv()

        scalars = rel_pos.as_quat(scalar_first=True)[:, 0]

        angles = 2 * np.arccos(np.clip(scalars, 0.0, np.pi / 2 - 1e-3))

        failures = angles > (np.pi / 4)

        return failures.astype(float).sum()
    
    def print_current_pairwise_contacts_with_target_obj(self, target_obj: Actor):
        contact_dict = {}

        # Check all objects
        for name, curr_obj in self.all_objects.items():
            forces = self.scene.get_pairwise_contact_forces(target_obj, curr_obj)
            contact_dict[name] = torch.linalg.norm(forces).cpu().item()

        # Check robot links
        for link_name in self.agent.finger_tip_links + self.agent.hand_link_names + [self.agent.palm_link, self.agent.floating_base_link]:
            link = self.agent.robot.find_link_by_name(link_name)
            forces = self.scene.get_pairwise_contact_forces(target_obj, link)
            contact_dict[f"finger_{link_name}"] = torch.linalg.norm(forces).cpu().item()

        # Check articulated objects
        for name, curr_art in self.all_articulations.items():
            for link in curr_art.links:
                forces = self.scene.get_pairwise_contact_forces(target_obj, link)
                contact_dict[f"art_{name}_{link.name}"] = torch.linalg.norm(forces).cpu().item()

        df = pd.DataFrame.from_dict(contact_dict, orient="index")
        with pd.option_context('display.max_rows', None, 'display.width', None):
            print(df)
            
    def print_current_pairwise_self_contacts_agent(self):
        relevant_links = self.agent.finger_tip_links + self.agent.hand_link_names + [self.agent.palm_link, self.agent.floating_base_link]
        contact_mat = []
        # Check robot links
        for curr_link_name in relevant_links:    
            curr_link = self.agent.robot.find_link_by_name(curr_link_name)
        
            contact_mat.append([torch.linalg.norm(self.scene.get_pairwise_contact_forces(curr_link, self.agent.robot.find_link_by_name(target_link_name))).cpu().item() for target_link_name in relevant_links])              

        df = pd.DataFrame(contact_mat, index=relevant_links, columns=relevant_links)
        with pd.option_context('display.max_rows', None, 'display.width', None):
            print(df)

    def print_all_contacts(self):
        contacts = self.scene.sub_scenes[0].get_contacts()  # CPU only

        for contact in contacts:
            print(f"Contact between: {contact.bodies[0].entity.name} and {contact.bodies[1].entity.name}")
